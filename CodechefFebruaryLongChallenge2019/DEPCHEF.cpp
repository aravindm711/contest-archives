...
v32 defense_values(v32 defenseval) {
	v32 x;

	int count = 0;
	forsn(j, 2, 5)
		if (defenseval[j] > 0) count++;
	...
	looptc {
		...
		forn(i, n) {
			...
			if (i == 0) {
				defenseval.pb(i);
				defenseval.pb(d[i]);
				defenseval.pb(d[i] - a[n - 1]);
				defenseval.pb(d[i] - a[1]);
				defenseval.pb(d[i] - a[n - 1] - a[1]);

				vallist = defense_values(defenseval);
			} else if (i == n - 1) {
				defenseval.pb(i);
				defenseval.pb(d[i]);
				defenseval.pb(d[i] - a[n - 2]);
				defenseval.pb(d[i] - a[0]);
				defenseval.pb(d[i] - a[n - 2] - a[0]);

				vallist = defense_values(defenseval);
			} else {
				defenseval.pb(i);
				defenseval.pb(d[i]);
				defenseval.pb(d[i] - a[i - 1]);
				defenseval.pb(d[i] - a[i + 1]);
				defenseval.pb(d[i] - a[i + 1] - a[i - 1]);
