...
int main() {
	...
	if (n == 1) {
		...
		unordered_map<int, int> inp_count;
		int inp;
		forn(i, 4) {
			cin >> inp;
			inp_count[inp]++;
		...
		if (inp_count.size() == 1) {
			cout << "yes" << ln;
			cout << inp << " " << inp << ln << inp << " " << inp << endl;
		} else {
			cout << "no" << endl;
		}
	} else {
		unordered_map<int, int> inp_count;
		v32 count4, count2, count1;
		int inp;
		for(int i = 0; i < (n*n); i++) {
			cin >> inp;
			inp_count[inp]++;
		...
		if ((n & 1) != 1 && (count4.size() != pow(n/2, 2))) {
			cout << "no";
		} else {
			plot_count4();
		}
		...
		if (n & 1) {
			for (auto i = inp_count.begin(); i != inp_count.end(); i++) {
				if (i->ss >= 2) {
					count2.pb(i->ss);
					i->ss %= 2;
				}
