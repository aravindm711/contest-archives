...
int main() {
	...
	// 	} // cout << temp << " " << num_list[i][n] << ln;
	// 	if (temp_sum == num_list[i][n])
	// 		count++;
	...
	v32 inp;
	int temp;
	int eSuf = 0, oSuf = 0, ePref = 0, oPref = 0;
	forn(i, n) {
		cin >> temp, inp.pb(temp);
	...
	forn(i, n) {
		...
		if (oPref + eSuf == ePref + oSuf) count++;
