...
int main() {
	...
	freopen("~/documents/coding/_input.txt", "r", stdin);
	// freopen("output.txt", "w", stdout);

	int n, t;
	cin >> n >> t;

	int temp1, temp2, index, count;
	v32 count_list;
	forn(i, n) {
		cin >> temp1 >> temp2;

		if (temp1 >= t) {
			count_list.pb(temp1-t);
		} else {
			count_list.pb(temp1+(temp2*ceil((float)(t - temp1)/temp2))-t);
		}
	}
	cout << (distance(count_list.begin(), min_element(count_list.begin(), count_list.end()))+1) << endl;

	// forn(i, n) {
	// 	cin >> temp1 >> temp2;

	// 	first.pb(temp1-t < 0 ? 2000000 : temp1-t);
	// 	sum.pb(temp1+temp2-t < 0 ? temp1+(temp2*ceil((t - temp1)/temp2)) : temp1+temp2-t);
	// }

	// auto minfirst = min_element(first.begin(), first.end());
	// auto minsum = min_element(sum.begin(), sum.end());
	// if (*minfirst <= *minsum)
	// 	cout << (distance(first.begin(), minfirst)+1) << endl;
	// else
	// 	cout << (distance(sum.begin(), minsum)+1) << endl;
