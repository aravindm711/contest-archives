...
int main() {
	...
	int* inp = new int[n];
	int64* sum = new int64[n];
	forn(i, n) sum[i] = 0;

	pair<long double, int> max;
	max.ss = 0;
	cin >> inp[0], sum[0] = inp[0], max.ff = inp[0];
	forsn(i, 1, n)
		cin >> inp[i], sum[i] += (inp[i] + sum[i - 1]), max.ff = (max.ff < inp[i]) ? inp[i] : max.ff;

	long double temp;
	for(int i = 2; i < n; i++) {
		for(int j = n - 1; j > 0; j--) {
			if(j - i >= 0) {
				temp = abs((long double)(sum[j] - sum[j - i])/i);
				if(max.ff < temp) {
					max.ff = temp;
					max.ss = i;
					cout << i << ln;
				} else if(max.ff == temp) {
					max.ss = (max.ss < i) ? i : max.ss;
				}
			} else break;
		}
	} 
	if ((long double)sum[n - 1]/(n) >= max.ff)
		cout << n << ln;
	else
		cout << ((max.ss == 0) ? 1 : max.ss);
